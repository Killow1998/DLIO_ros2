...
void dlio::MapNode::getParams() {
  ...
  this->declare_parameter<int>("map/dense_input/skip", 5);

  this->get_parameter("odom/odom_frame", this->odom_frame);
  this->get_parameter("map/sparse/leafSize", this->leaf_size_);
  this->get_parameter("map/dense_input/skip", this->dense_input_skip);
  this->dense_input_cnt = 0;
}
...
void dlio::MapNode::callbackKeyframe(
  ...
  // if using dense input, only process every Nth input
  dense_input_cnt++;
  if (dense_input_skip == dense_input_cnt) {
    dense_input_cnt = 0;
  } else {
    return;
  }
  ...
  vg.setLeafSize(this->leaf_size_, this->leaf_size_, this->leaf_size_);
  vg.setInputCloud(keyframe_pcl);
  vg.filter(*keyframe_pcl);

  // save filtered keyframe to map for rviz
  *this->dlio_map += *keyframe_pcl;

  // publish full map
  // if (this->dlio_map->points.size() == this->dlio_map->width *
  // this->dlio_map->height)
